...
% ======================= LỌC DẢI THÔNG  TƯƠNG TỰ CẦN THIẾT KẾ ======================
fc_p1 = 1;                      % Tần số cắt thấp hơn của dải thông (Hz)
OMEGAc_p1 = 2 * pi * fc_p1;     % Tần số cắt thấp hơn của dải thông (rad/s)
fc_p2 = 3;                      % Tần số cắt cao hơn của dải thông (Hz)
OMEGAc_p2 = 2 * pi * fc_p2;     % Tần số cắt cao hơn của dải thông (rad/s)
s1 = OMEGA_c * (sp^2 + OMEGAc_p1 * OMEGAc_p2) / (sp * (OMEGAc_p2 - OMEGAc_p1));
Ha_s1 = subs(Ha_sTQ, s1);

fc_p1 = 10;                      % Tần số cắt thấp hơn của dải thông (Hz)
OMEGAc_p1 = 2 * pi * fc_p1;     % Tần số cắt thấp hơn của dải thông (rad/s)
fc_p2 = 13;                      % Tần số cắt cao hơn của dải thông (Hz)
OMEGAc_p2 = 2 * pi * fc_p2;     % Tần số cắt cao hơn của dải thông (rad/s)
% Biểu thức chuyển đổi từ bộ lọc thấp qua sang bộ lọc dải thông
s2 = OMEGA_c * (sp^2 + OMEGAc_p1 * OMEGAc_p2) / (sp * (OMEGAc_p2 - OMEGAc_p1));
Ha_s2 = subs(Ha_sTQ, s2);
Ha_s = Ha_s1 +Ha_s2;
