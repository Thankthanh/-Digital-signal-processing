...
fc_p1 = 3;                      % Tần số cắt thấp hơn của dải chặn (Hz)
OMEGAc_p1 = 2 * pi * fc_p1;     % Tần số cắt thấp hơn của dải chặn (rad/s)
fc_p2 = 11;                      % Tần số cắt cao hơn của dải chặn (Hz)
OMEGAc_p2 = 2 * pi * fc_p2;     % Tần số cắt cao hơn của dải chặn (rad/s)

% Biểu thức chuyển đổi từ bộ lọc thấp qua sang bộ lọc dải thông
s = OMEGA_c * ( sp * (OMEGAc_p2 - OMEGAc_p1) /(sp^2 + OMEGAc_p1 * OMEGAc_p2) );

% Thay đổi biến s vào hàm chuyển của lọc thấp qua tương tự để nhận được hàm chuyển của bộ lọc dải thông
Ha_s = subs(Ha_sTQ, s);


% =========================== DẢI CHẶN QUA SỐ CẦN THIẾT KẾ =========================
% -------------- Hàm truyền đạt H(z)
sz = 2/ts * (1-u)/(1+u);
Hz = subs(Ha_s,sz);
